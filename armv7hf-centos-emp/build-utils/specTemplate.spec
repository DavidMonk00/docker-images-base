...
if [ "${#DEBUGDIR}" -gt "${#PKGDIR}" ]; then
  (>&2 echo "ERROR : Debuginfo destination directory path '${DEBUGDIR}' is longer than source code directory path '${PKGDIR}'.")
  (>&2 echo "Please make base directory path of source code at least $((${#DEBUGDIR} - ${#PKGDIR})) characters longer in order to build RPMs.")
  exit 1
fi
...
# find . -name "*" -exec install -D -m 755 {} $RPM_BUILD_ROOT/%{_prefix}/bin/{} \;
  find . -name "*" -exec __buildutils_dir__/install.sh {} %{_prefix}/bin/%{_project}/{} 755 $RPM_BUILD_ROOT %{_packagedir} %{_packagename} %{_version} %{_prefix}/include '%{_includedirs}' \;
fi
...
#  find . -name ".svn" -prune -o -name "*" -exec install -D -m 644 {} $RPM_BUILD_ROOT/%{_prefix}/lib/{} \;
  find . -name "*" -exec __buildutils_dir__/install.sh {} %{_prefix}/lib/{} 655 $RPM_BUILD_ROOT %{_packagedir} %{_packagename} %{_version} %{_prefix}/include '%{_includedirs}' \;
fi
